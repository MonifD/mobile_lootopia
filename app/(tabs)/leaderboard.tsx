...
const standardLoaders: Partial<
  Record<LeaderboardMode, () => Promise<LeaderboardEntry[]>>
> = {
...
export default function LeaderboardScreen() {
  ...
  const loadLeaderboardStats = useCallback(
    () => lootopiaApi.getLeaderboardStats(),
    [],
  );
  const loadByHunts = useCallback(
    () => lootopiaApi.getLeaderboardByHunts(),
    [],
  );
  ...
  const byHuntsState = useApiResource<LeaderboardEntry[]>(loadByHunts);

  useEffect(() => {
    void entriesState.refresh();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [mode]);

  const huntsMap = useMemo(() => {
    const map = new Map<number, number>();
    for (const e of byHuntsState.data ?? []) {
      map.set(e.id, e.completedHunts);
    }
    return map;
  }, [byHuntsState.data]);
  ...
  const entries = useMemo(
    () =>
      (entriesState.data ?? []).map((e) =>
        mode === "global" && huntsMap.has(e.id)
          ? { ...e, completedHunts: huntsMap.get(e.id)! }
          : e,
      ),
    [entriesState.data, huntsMap, mode],
  );

  const topThree = useMemo(() => entries.slice(0, 3), [entries]);

  // Derived fallbacks when the dedicated API endpoints fail
  const displayTotalPlayers =
    statsState.data?.totalPlayers ||
    (entries.length > 0 ? entries.length : null);

  const displayAveragePoints = useMemo(() => {
    if (statsState.data?.averagePoints) return statsState.data.averagePoints;
    if (entries.length === 0) return null;
    const sum = entries.reduce((acc, e) => acc + e.totalPoints, 0);
    return Math.round(sum / entries.length);
  }, [statsState.data, entries]);

  const displayMyRank = useMemo(() => {
    if (myRankState.data?.rank) return myRankState.data.rank;
    if (!session?.username) return null;
    const found = entries.find(
      (e) => e.username.toLowerCase() === session.username.toLowerCase(),
    );
    return found?.rank ?? null;
  }, [myRankState.data, entries, session?.username]);
      ...
      >
        ...
        <GoldFrame>
          ...
          <Text style={styles.heroText}>
            Compare tes points, grimpe dans le top et deviens une légende
            Lootopia.
          </Text>
        </GoldFrame>
        ...
        <View style={styles.statsGrid}>
          ...
          <StatCard
            icon="🏴‍☠️"
            label="MON RANG"
            value={displayMyRank != null ? `#${displayMyRank}` : "-"}
            color="#5a3708"
          />
              ...
              >
                ...
                >
                  ...
                  <View style={styles.pointsBadge}>
                    ...
                    <Text style={styles.entryPointsValue}>
                      {entry.totalPoints}
                    </Text>
                  </View>
                </LinearGradient>
